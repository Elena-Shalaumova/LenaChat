...
import com.example.easybot.data.remote.api.ChatSettingsDto
import com.example.easybot.data.remote.api.CreateChatRequest
import com.example.easybot.data.remote.api.MessageDto
import com.example.easybot.data.remote.api.SendMessageRequest
...
import com.example.easybot.data.remote.dto.SingleChatExportDto
...
class ChatRepository(
    ...
    suspend fun createChat(title: String, isIncognito: Boolean): ChatDto {
        val request =
            CreateChatRequest(title = title, userId = getUserId(), isIncognito = isIncognito)
    ...
    suspend fun exportSingleChatToJson(
        ...
        try {
            ...
            e.printStackTrace()
            Log.e("EXPORT_DEBUG", "Ошибка экспорта одного чата", e)
            Toast.makeText(
                context,
